...
class Matrix {
	...
	private function ft_make_projection($far, $near, $fov, $ratio)
	{
		...
		echo "le near ".$near;
		echo "le fov". $fov;
		echo "le roation".$ratio;
		/* $ratio = $this->_height / $this->transformMesh(width; */
		$fov = deg2rad($fov);
		$fov_rad = 1.0 / tan($fov / 2.0);
		$this->_matrix[3][2] = 2 * $near * $far / ($near - $far);
		$this->_matrix[2][2] = -1 * (-$near - $far) / ($near - $far);
	...
	//ajout pour exo 5
	public function transformMesh($array_of_triangle)// : Vertex  vertex multiply matrix
	{
		$result_array_of_triangle = array();
		foreach ($array_of_triangle as $triangle)
		{
			$a = $this->transformVertex($triangle->_A);
			$b = $this->transformVertex($triangle->_B);
			$c = $this->transformVertex($triangle->_C);
